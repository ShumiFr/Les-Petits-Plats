 ...
 * @param {string} filterType - Le type de filtre appliqué.
 */
export function refreshDropdownItems(
  items,
  dropdown,
  searchInput,
  dropdownItemContainer,
  ...
  activeItemsContainer.querySelectorAll(".filter-delete").forEach((button) => {
    button.addEventListener("click", (event) => {
      ...
      const filterToRemove = event.target
        .closest("li")
 ...
 * @param {string} filterType - Le type de filtre appliqué.
 */
function addSelectedItemToActiveFilters(
  event,
  items,
  dropdown,
  searchInput,
...
export function initializeDropdown(config, items) {
  ...
  dropdownItemContainer.addEventListener("click", (event) => {
    addSelectedItemToActiveFilters(
      event,
      items,
      dropdown,
      searchInput,
 ...
 */
export function extractUniqueItems(recipes, key) {
